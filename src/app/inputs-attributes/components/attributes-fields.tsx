...
import { InputsAttributesData } from '@/schemas/inputs-attributes-schema'
...
export default function AttributesFields({ nestIndex, control }: Props) {
  const { fields, append, remove } = useFieldArray({
    control,
    name: `inputsAttributes.${nestIndex}.attributes`,
  })
  return (
    <>
      <Button
      ...
      {fields.map((field, index) => (
        <div className="mt-3 flex items-center gap-3" key={field.id}>
          <FormField
            control={control}
            name={`inputsAttributes.${nestIndex}.attributes.${index}.name`}
            render={({ field }) => (
              <FormItem>
                <FormControl>
          ...
          <FormField
            control={control}
            name={`inputsAttributes.${nestIndex}.attributes.${index}.type`}
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Select
